    ...
    if (authService.isAuthenticated()) {
      ...
      setTimeout(() => {
        if (user.roles?.includes('Admin')) {
          router.push('/admin');
        } else if (user.roles?.includes('HR') && user.roles?.includes('Interviewer')) {
          // User has both HR and Interviewer roles - default to HR page with toggle
          router.push('/hr');
        } else if (user.roles?.includes('HR')) {
          router.push('/hr');
        } else if (user.roles?.includes('Interviewer') || user.roles?.includes('Technical Team') || user.roles?.includes('Management')) {
    ...
    try {
      ...
      setTimeout(() => {
        if (result.user.roles?.includes('Admin')) {
          router.push('/admin');
        } else if (result.user.roles?.includes('HR') && result.user.roles?.includes('Interviewer')) {
          // User has both HR and Interviewer roles - default to HR page with toggle
          router.push('/hr');
        } else if (result.user.roles?.includes('HR')) {
          router.push('/hr');
        } else if (result.user.roles?.includes('Interviewer') || result.user.roles?.includes('Technical Team') || result.user.roles?.includes('Management')) {
          router.push('/interviewer');
        } else {
          setError('No valid role assigned. Please contact administrator.');
        }
      }, 200);
    } catch (err) {
          ...
          <Card className="w-full transform transition-all duration-300 hover:shadow-2xl shadow-xl animate-fade-in-up">
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {error && (
                  <div className="text-sm text-red-600 bg-red-50 p-3 rounded-md transition-all duration-300">
                    {error}
                  </div>
                )}
                <Button
                  type="submit"
                  className="w-full transition-all duration-200 hover:shadow-lg transform hover:scale-[1.02]"
                  disabled={loading}
                >
                  {loading ? (
